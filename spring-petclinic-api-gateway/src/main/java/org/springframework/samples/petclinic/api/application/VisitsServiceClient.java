...
public class VisitsServiceClient {
	...
	public Map<String, List<VisitDetails>> getVisitsForPets(final List<String> petIds, final String ownerId) {
		...
		return petIds.parallelStream().flatMap(petId -> loadBalancedRestTemplate
				.exchange("http://visits-service/owners/{ownerId}/pets/{petId}/visits", HttpMethod.GET, null, responseType, ownerId, petId)
				.getBody().stream()).collect(groupingBy(VisitDetails::getPetId));
	}
}
